--!nonstrict
------------------------------------------------------------------------
-- Freecam (Adaptado para Xeno) - SEM texto no canto
-- Toggle: Shift + P
------------------------------------------------------------------------
...
local FFlagUserFixFreecamDeltaTimeCalculation = true
local FFlagUserFixFreecamGuiChangeVisibility = true
...
local FFlagUserFreecamPlayerLock = true
local FFlagUserFreecamPlayerLockResetHeight = true
...
local Input = {} do
	...
	local gamepad = {
		ButtonX = 0, ButtonY = 0,
		DPadDown = 0, DPadUp = 0, DPadLeft = 0, DPadRight = 0,
	...
	local keyboard = {
		...
		Z = 0, C = 0, Comma = 0, Period = 0,
		LeftBracket = 0, RightBracket = 0,
		Semicolon = 0, Quote = 0, V = 0, B = 0, N = 0, M = 0,
		BackSlash = 0, Minus = 0, Equals = 0,
		Slash = 0, R = 0, T = 0, X = 0, L = 0
	}
	...
	do
		local function resetKeys(keys, tbl)
			for keyEnum, _ in pairs(keys) do
				if tbl[keyEnum.Name] then 
					tbl[keyEnum.Name] = 0
				end
			end
		end
...
local function StepFreecam(dt)
	...
	local panVector = pan*PAN_GAIN*(dt/zoomFactor)
	...
	local cameraCFrame = CFrame.new(cameraPos)*CFrame.fromOrientation(cameraRot.x, cameraRot.y, cameraRot.z)*CFrame.new(vel*NAV_GAIN*dt)
...
do
	...
	local setCores = {
		BadgesNotificationsActive = true, PointsNotificationsActive = true,
	}
...
do
	...
	local function ToggleFreecam()
		if enabled then
			...
			StartFreecam()
			print("[Freecam] Ativado - Shift+P para desligar")
		end
		enabled = not enabled
	end
...
print("[Freecam] Carregado! Pressione Shift + P para ativar")
